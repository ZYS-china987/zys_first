...
def main():
    ...
    # 训练集上的表现
    pre_train = svm.predict(x_train_Scaler)
    ...
    return y_test.ravel(), pre, RMSE, MAE, MAPE, MAPE_train, pre_train


if __name__ == '__main__':
    pre = np.zeros([30, 100])
    pre_train = np.zeros([30, 70])
    true = np.zeros([30, 100])
    RMSE = np.zeros([30, 1])
    MAE = np.zeros([30, 1])
    MAPE = np.zeros([30, 1])
    MAPE_train = np.zeros([30, 1])
    for i in range(30):
        ...
        MAE[i, :] = results[3]
        MAPE[i, :] = results[4]
        MAPE_train[i, :] = results[5]
        pre_train[i, :] = results[6]
    np.savetxt('true.txt', true)
    np.savetxt('pre.txt', pre)
    np.savetxt('pre_train.txt', pre_train)
